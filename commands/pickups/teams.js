const run = async(message) => {
    ...
    const teams = pugger.game.teams.toPuggers();
    teams.alpha.avgElo = teams.alpha.reduce((s, p) => s += p.globalElo, 0) / teams.alpha.length;
    teams.beta.avgElo = teams.beta.reduce((s, p) => s += p.globalElo, 0) / teams.beta.length;
